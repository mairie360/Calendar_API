...
# ghcr.io/mairie360/calendar-api:dev-<sha> image (the one promoted to staging and prod); when it is
# empty (local usage) the image is built from development.Dockerfile as calendar-api:local.

COMPOSE_FILE="docker-compose-security.yml"
SERVICE_NAME="security-scan"

if [ -z "${IMAGE_REF:-}" ]; then
    echo "==> [0/4] IMAGE_REF is empty: building calendar-api:local from development.Dockerfile..."
    IMAGE_REF="calendar-api:local"
